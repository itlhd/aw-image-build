...
compile_kernel()
{
	...
	# 编译内核deb包
	display_alert "Compiler kernel deb" "make $CTHREADS ARCH=$ARCH bindeb-pkg KDEB_COMPRESS=xz LOCALVERSION='' KDEB_PKGVERSION=$REVISION CROSS_COMPILE=$CCACHE $LINUX_COMPILER"  "info"
	eval CCACHE_BASEDIR="$(pwd)" \
		'make $CTHREADS ARCH=$ARCH bindeb-pkg KDEB_COMPRESS=xz \
